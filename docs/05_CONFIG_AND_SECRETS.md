...
## 6. MS-05.04 OAuth smoke test 로컬 설정

실제 OAuth smoke test가 승인된 경우에만 사용자가 `.env.local`을 직접 생성하고
아래 placeholder에 실제 값을 로컬에서 입력합니다.
...
- 실제 값을 채팅, report, log, git diff에 기록하지 않습니다.
- `.env.local`은 `.gitignore` 대상이며 commit하지 않습니다.
- `accountSeq`와 기존 Access Token은 이번 단계에서 필요하지 않습니다.
- 발급된 token은 프로세스 메모리에서만 사용하고 파일에 저장하지 않습니다.
- 출력은 `safe_dict()`의 masking 결과만 허용합니다.
- 실패 시 request form, response body, Client ID/Secret 원문을 출력하지 않습니다.
