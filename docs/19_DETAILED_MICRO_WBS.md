...
| MS-08.02 | Mock-only recommendation policy model | src/ai_stock/recommendation/, tests/ | mock/local snapshot DTO 기반 deterministic policy와 explanation contract | 실제 AI/LLM/API refresh·투자 자문·직접 매수/매도 지시·실주문 없이 별도 승인 후 진행 |
| MS-08.03 | Recommendation explanation UI preflight | src/ai_stock/recommendation/, tests/, reports/ | explanation-only UI boundary and sensitive-action preflight | 기존 read-only dashboard와 실거래 경계를 유지하며 별도 승인 후 진행 |
