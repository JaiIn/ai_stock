...
| MS-08.04 | Mock-only recommendation panel UI integration | app/streamlit_app.py, src/ai_stock/ui/, tests/, reports/ | MS-08.03 UI contract를 따르는 mock-only explanation panel 연결. 별도 승인 전까지 실행하지 않음 | 실제 추천, 투자 자문, buy/sell/hold 지시, live refresh, OAuth, credential/accountSeq 입력, order/account/assets/balance/fills, DB write 및 실주문 버튼은 계속 금지 |
