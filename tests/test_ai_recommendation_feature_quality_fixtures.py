"""Offline tests for the MS-10.01 feature quality fixture expansion."""

from dataclasses import FrozenInstanceError, replace
from types import FunctionType
import unittest

import ai_stock.recommendation.feature_quality_fixtures as quality_fixtures
...
class FeatureQualityFixtureExpansionTests(unittest.TestCase):
    def setUp(self) -> None:
        self.policy = build_feature_quality_fixture_policy()

    def test_feature_quality_fixture_policy_generation(self) -> None:
        self.assertEqual(self.policy.stage_name, "MS-10.01")
        self.assertEqual(self.policy.mode, "feature_quality_fixture_expansion")
        self.assertEqual(
    ...
    def test_all_fixture_scenarios_are_generated(self) -> None:
        ...
        self.assertEqual(
            ...
            tuple(scenario.value for scenario in FeatureQualityFixtureScenario),
        )
    ...
    def test_all_fixture_scenarios_are_deterministic(self) -> None:
        self.assertEqual(
            build_all_feature_quality_fixtures(),
            build_all_feature_quality_fixtures(),
        )
    ...
    def test_all_fixture_evaluators_pass(self) -> None:
        evaluations = evaluate_all_feature_quality_fixtures()
    ...
    def test_quality_basic_ok_processing(self) -> None:
        evaluation = evaluate_feature_quality_fixture(
            build_quality_basic_ok_fixture()
        )

        self.assertTrue(evaluation.passed)
        self.assertEqual(evaluation.actual_quality_statuses, ("quality_ok", "quality_ok"))
    ...
    def test_quality_mixed_invalid_and_review_processing(self) -> None:
        evaluation = evaluate_feature_quality_fixture(
            build_quality_mixed_invalid_and_review_fixture()
        )
        ...
        self.assertTrue(evaluation.passed)
        self.assertEqual(
            evaluation.actual_quality_statuses,
            (
                "quality_ok",
    ...
    def test_quality_duplicates_blocked_processing(self) -> None:
        evaluation = evaluate_feature_quality_fixture(
            build_quality_duplicates_blocked_fixture()
        )

        self.assertTrue(evaluation.passed)
        self.assertIn("quality_duplicate_candidate", evaluation.actual_quality_statuses)
    ...
    def test_quality_disabled_blocked_processing(self) -> None:
        evaluation = evaluate_feature_quality_fixture(
            build_quality_disabled_blocked_fixture()
        )

        self.assertTrue(evaluation.passed)
        self.assertIn("quality_disabled_candidate", evaluation.actual_quality_statuses)
    ...
    def test_quality_insufficient_data_blocked_processing(self) -> None:
        evaluation = evaluate_feature_quality_fixture(
            build_quality_insufficient_data_blocked_fixture()
        )
        ...
        self.assertEqual(evaluation.actual_usable_for_future_scoring_count, 0)
        self.assertIn("insufficient_data_review", evaluation.actual_warnings)

    def test_quality_forbidden_field_sanitized_processing(self) -> None:
        evaluation = evaluate_feature_quality_fixture(
            build_quality_forbidden_field_sanitized_fixture()
        )
        ...
        for keyword in FORBIDDEN_FEATURE_QUALITY_OUTPUT_KEYWORDS:
            self.assertNotIn(keyword, rendered)

    def test_quality_empty_input_processing(self) -> None:
        evaluation = evaluate_feature_quality_fixture(
            build_quality_empty_input_fixture()
        )

        self.assertTrue(evaluation.passed)
        self.assertEqual(evaluation.actual_quality_statuses, ("quality_empty_input",))
        self.assertEqual(evaluation.actual_needs_review_count, 1)
        self.assertIn("empty_source_items", evaluation.actual_diagnostics)

    def test_quality_all_fixture_matrix_processing(self) -> None:
        evaluation = evaluate_feature_quality_fixture(
            build_quality_all_fixture_matrix()
        )

        self.assertTrue(evaluation.passed)
        self.assertEqual(len(evaluation.actual_quality_statuses), 11)
    ...
    def test_expected_vs_actual_mismatch_generates_failure(self) -> None:
        fixture = build_quality_basic_ok_fixture()
        mismatched = replace(
            fixture,
            expected_quality_statuses=("quality_empty_input",),
        )
        evaluation = evaluate_feature_quality_fixture(mismatched)

        self.assertFalse(evaluation.passed)
        self.assertIn("quality_statuses_mismatch", evaluation.failures)

    def test_required_flags_are_all_false(self) -> None:
        for evaluation in evaluate_all_feature_quality_fixtures():
            self.assertEqual(
    ...
    def test_forbidden_action_status_is_not_generated_as_output(self) -> None:
        ...
        for evaluation in evaluate_all_feature_quality_fixtures():
            produced_values.extend(evaluation.actual_quality_statuses)
    ...
    def test_forbidden_field_values_are_not_copied_to_output_model(self) -> None:
        rendered = repr(evaluate_all_feature_quality_fixtures())
    ...
    def test_module_structure_has_no_io_or_external_dependencies(self) -> None:
        runtime_modules = {
            ...
            for value in vars(quality_fixtures).values()
        }
    ...
    def test_module_has_no_forbidden_runtime_imports_or_paths(self) -> None:
        module_symbols = {
            str(part).casefold()
            for part in (*vars(quality_fixtures), *_fixture_module_constants())
        }
        for forbidden in (
            "import streamlit",
            "ai_stock.clients",
    ...
    def test_existing_ui_and_contract_modules_are_not_referenced_as_paths(self) -> None:
