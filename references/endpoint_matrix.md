...
| Market Data | getCandles | GET | `/api/v1/candles` | Yes | No | Yes | MS-05.14 schema-aligned; future single live candidate | Required symbol/interval validation, optional count/before/adjusted handling, official CandlePage parser, and safe error metadata implemented | High | `interval` enum is `1m`/`1d`; count is optional default 100 with 1~200 range; next candidate is `symbol=005930&interval=1d&count=1&adjusted=true` and was not executed in MS-05.14 | Official OpenAPI 1.1.5 `/api/v1/candles` |
