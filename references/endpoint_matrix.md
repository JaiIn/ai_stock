...
| Market Info | getExchangeRate | GET | `/api/v1/exchange-rate` | Yes | No | Yes | MS-05.06 single diagnostic retry completed | Schema-aligned parser, safety-gated smoke client, and safe phase/status diagnostics implemented | High | One approved retry reached `readonly_exchange_rate` and returned HTTP 400 with safe code `invalid-request`; no raw response/token storage and no further retry | Official OpenAPI `/api/v1/exchange-rate` |
